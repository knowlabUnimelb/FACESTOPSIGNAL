% 29-07-20 DL
% Figure E1
% 6: S/S to L/L, L/H, H/L, H/H --> Is there an interaction between changing
...
for sidx = 1:numel(subs)
    ...
    hits =  [hits; 
             ...
             subs(sidx), conditions(sidx), 2, subHR(subHR(:,strcmp(cols, 'top')) == 2 & subHR(:,strcmp(cols, 'bot')) == 3, strcmp(cols, 'Nresp'))';
             subs(sidx), conditions(sidx), 3, subHR(subHR(:,strcmp(cols, 'top')) == 3 & subHR(:,strcmp(cols, 'bot')) == 2, strcmp(cols, 'Nresp'))';
             subs(sidx), conditions(sidx), 4, subHR(subHR(:,strcmp(cols, 'top')) == 3 & subHR(:,strcmp(cols, 'bot')) == 3, strcmp(cols, 'Nresp'))'];
    
    Ns = [Ns;
          subs(sidx), conditions(sidx), 1, subHR(subHR(:,strcmp(cols, 'top')) == 2 & subHR(:,strcmp(cols, 'bot')) == 2, strcmp(cols, 'Ntot'))';
          subs(sidx), conditions(sidx), 2, subHR(subHR(:,strcmp(cols, 'top')) == 2 & subHR(:,strcmp(cols, 'bot')) == 3, strcmp(cols, 'Ntot'))';
          subs(sidx), conditions(sidx), 3, subHR(subHR(:,strcmp(cols, 'top')) == 3 & subHR(:,strcmp(cols, 'bot')) == 2, strcmp(cols, 'Ntot'))';
          subs(sidx), conditions(sidx), 4, subHR(subHR(:,strcmp(cols, 'top')) == 3 & subHR(:,strcmp(cols, 'bot')) == 3, strcmp(cols, 'Ntot'))'];
    ...
    falarms = [falarms; 
        ...
        subs(sidx), conditions(sidx), 3, subFA(subFA(:,strcmp(cols, 'top')) == 1 & subFA(:,strcmp(cols, 'bot')) == 1, strcmp(cols, 'Nresp'))';
        subs(sidx), conditions(sidx), 4, subFA(subFA(:,strcmp(cols, 'top')) == 1 & subFA(:,strcmp(cols, 'bot')) == 1, strcmp(cols, 'Nresp'))'];
    ...
    Nn = [Nn; 
        ...
        subs(sidx), conditions(sidx), 3, subFA(subFA(:,strcmp(cols, 'top')) == 1 & subFA(:,strcmp(cols, 'bot')) == 1, strcmp(cols, 'Ntot'))';
        subs(sidx), conditions(sidx), 4, subFA(subFA(:,strcmp(cols, 'top')) == 1 & subFA(:,strcmp(cols, 'bot')) == 1, strcmp(cols, 'Ntot'))'];
...
%% Run JAGS
savefn = sprintf('samples_topDiffBottomDiff_s%d.mat', si);
...
colours = [.10 .72 .88; .27 .47 .77; .43 .27 .66; .56 .03 .52;
           ...
           %.87 .25 .45; .67 .53 .54; .54 .76 .60; .44 .99 .69];
           
           %.93 .19 .47; .95 .47 .37; .96 .69 .28; .97 .93 .22] % pink to yellow
...
subplot(2,2,1)
fh11 = fill([xt, fliplr(xt)], [low11, fliplr(hi11)], [.10 .72 .88], 'FaceAlpha', .4, 'EdgeAlpha', 0); hold on
fh12 = fill([xt, fliplr(xt)], [low12, fliplr(hi12)], [.27 .47 .77], 'FaceAlpha', .4, 'EdgeAlpha', 0);
fh13 = fill([xt, fliplr(xt)], [low13, fliplr(hi13)], [.43 .27 .66], 'FaceAlpha', .4, 'EdgeAlpha', 0);
fh14 = fill([xt, fliplr(xt)], [low14, fliplr(hi14)], [.56 .03 .52], 'FaceAlpha', .4, 'EdgeAlpha', 0);
%vh11 = violin(post_d11, 'x', xloc, ...
 %   'facecolor', [1 1 1], 'edgecolor', [.5 0 0], 'facealpha', 1); hold on
%vh12 = violin(post_d12, 'x', xloc, ...
 %   'facecolor', [1 1 1], 'edgecolor', [0 0 .5], 'facealpha', 1);
%vh13 = violin(post_d13, 'x', xloc, ...
 %   'facecolor', [1 1 1], 'edgecolor', [.5 0 .5], 'facealpha', 1);
%vh14 = violin(post_d14, 'x', xloc, ...
 %   'facecolor', [1 1 1], 'edgecolor', [0 .5 .5], 'facealpha', 1);
vh11 = violin(post_d11, 'x', xloc, ...
    'facecolor', [.10 .72 .88], 'edgecolor', 'k', 'facealpha', .7); hold on
vh12 = violin(post_d12, 'x', xloc, ...
    'facecolor', [.27 .47 .77], 'edgecolor', 'k', 'facealpha', .7);
vh13 = violin(post_d13, 'x', xloc, ...
    'facecolor', [.43 .27 .66], 'edgecolor', 'k', 'facealpha', .7);
vh14 = violin(post_d14, 'x', xloc, ...
    'facecolor', [.56 .03 .52], 'edgecolor', 'k', 'facealpha', .7);
...
ylabel('d-prime')
legend([fh14, fh13, fh12, fh11],...
    'Aligned: High/High', 'Aligned: High/Low', 'Aligned: Low/High', 'Aligned: Low/Low',...
    'Location', 'Best')


subplot(2,2,2)
fh21 = fill([xt, fliplr(xt)], [low21, fliplr(hi21)], [.44 .99 .69], 'FaceAlpha', .5, 'EdgeAlpha', 0); hold on
fh22 = fill([xt, fliplr(xt)], [low22, fliplr(hi22)], [.54 .76 .60], 'FaceAlpha', .5, 'EdgeAlpha', 0);
fh23 = fill([xt, fliplr(xt)], [low23, fliplr(hi23)], [.67 .53 .54], 'FaceAlpha', .5, 'EdgeAlpha', 0);
fh24 = fill([xt, fliplr(xt)], [low24, fliplr(hi24)], [.87 .25 .45], 'FaceAlpha', .5, 'EdgeAlpha', 0);
...
%vh24 = violin(post_d24, 'x', xloc, ...
 ...
 vh21 = violin(post_d21, 'x', xloc, ...
    'facecolor', [.44 .99 .69], 'edgecolor', 'k', 'facealpha', .7); hold on
vh22 = violin(post_d22, 'x', xloc, ...
    'facecolor', [.54 .76 .60], 'edgecolor', 'k', 'facealpha', .7);
vh23 = violin(post_d23, 'x', xloc, ...
...
xlabel('Total Processing Time (sec)')
ylabel('d-prime')
legend([fh24, fh23, fh22, fh21],...
    'Misaligned: High/High', 'Misaligned: High/Low', 'Misaligned: Low/High', 'Misaligned: Low/Low',...
    'Location', 'Best')
